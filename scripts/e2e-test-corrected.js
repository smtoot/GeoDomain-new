...
/**
 * GeoDomain E2E Test Suite - Corrected Version
 * 
 * Comprehensive testing of all major functionality using proper Puppeteer methods
 */
...
class AuthTests {
  ...
  async testAdminLogin() {
    ...
    // Fill login form
    await this.page.type('input[type="email"]', 'admin@geodomain.com');
...
class DomainTests {
  ...
  async testDomainFilters() {
    ...
    if (categoryFilter) {
      await categoryFilter.select('Technology Services');
      await this.page.waitForSelector('body');
    ...
    if (stateFilter) {
      await stateFilter.select('California');
      await this.page.waitForSelector('body');
