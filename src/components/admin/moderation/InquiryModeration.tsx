...
interface InquiryModerationProps {
  inquiry: {
    id: string;
    buyerName: string;
    buyerEmail: string;
  ...
  onModerate: (inquiryId: string, action: 'APPROVE' | 'REJECT' | 'REQUEST_CHANGES', data: any) => Promise<void>;
}

export function InquiryModeration({ inquiry, onModerate }: InquiryModerationProps) {
  ...
  const handleModeration = async () => {
    ...
    try {
      await onModerate(inquiry.id, action, {
        adminNotes: notes,
      ...
      toast.error('Failed to moderate inquiry');
    } finally {
