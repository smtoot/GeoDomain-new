...
export const inquiriesRouter = createTRPCRouter({
  ...
  // Get seller's domain inquiries (admin approved only)
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const message = await ctx.prisma.message.create({
        data: {
          ...
          receiverId: isBuyer ? inquiry.sellerId : inquiry.buyerId,
      ...
      return {
        ...
        message: 'Message sent and is under review',
      };
    }),
  ...
  moderateMessage: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      switch (input.action) {
        case 'APPROVE':
          newStatus = 'APPROVED';
          adminNotes = adminNotes || 'Message approved';
          break;
        case 'REJECT':
          newStatus = 'REJECTED';
          if (!input.rejectionReason) {
